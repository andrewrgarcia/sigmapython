...
def one(label='',column='B2'):
    idx = args["sheet"]
    book=xw.Book(args["path"])    
    column_data = book.sheets[idx].range( column + ':' + column[0]+str(lastRow(idx,book)) ).value

    make(column_data,label=args["label"],dists=args["distribution"],bins=args["bins"],plots=True,xlims='')
    book.close()
